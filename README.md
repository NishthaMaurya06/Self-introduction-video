Self Introduction Video

Hello! I'm Nishtha Maurya

This repository contains my self-introduction video where I talk about my skills, projects, and background.

About Me
- B.Tech CSE student at VIT Bhopal University
- Skilled in Java, C++, DSA (C), Python
- Web Development: HTML, CSS, JavaScript

Projects
- Skin Cancer Detection using CNN
- Phishing Detection Model (ongoing)

Goal
To strengthen my technical skills and build a successful career in the IT field.

Click on the file "self_introduction.mp4" above and select "View raw" to watch the video.

